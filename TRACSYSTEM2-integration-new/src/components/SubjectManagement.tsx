...
export function SubjectManagement() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Subjects List (Styled like UserManagement) */}
      <Card className={outlineDarkBrownBox}>
        <CardContent className="p-0">
          {/* Header Row */}
          <div className="grid grid-cols-5 bg-white font-semibold rounded-t-2xl p-4 border-b">
            <div>Subject ID</div>
          ...
          <div className="space-y-3 p-4">
            {filteredSubjects.map((subject) => (
              <div
                key={subject.id}
                className="grid grid-cols-5 bg-gray-100 rounded-2xl p-4 items-center"
              >
                <div>{subject.id}</div>
                <div>{subject.name}</div>
