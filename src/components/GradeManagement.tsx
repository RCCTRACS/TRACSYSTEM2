...
export function GradeManagement() {
  ...
  const API_URL = "http://192.168.1.13/capstone/mainsystem/backend/grade_api.php";
    ...
    try {
      if (selectedGrade) {
        ...
        const res = await fetch(API_URL, {
          ...
          body: JSON.stringify({ id: selectedGrade.id, grade: gradeData.grade }),
        });
        const data = await res.json();
        if (data.success) {
        ...
        const res = await fetch(API_URL, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ grade: gradeData.grade }),
        });
        const data = await res.json();
        if (data.success) {
  ...
  const handleBulkUpload = async (file: File) => {
    ...
    for (let row of rows.slice(1)) {
      ...
      const grade = cols[headers.indexOf("grade")]?.trim();
      if (grade) {
        await fetch(API_URL, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
