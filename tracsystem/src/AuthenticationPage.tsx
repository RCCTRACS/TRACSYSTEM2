...
const AuthenticationPage: React.FC = () => {
  ...
  const sendOtp = async () => {
    try {
      const res = await fetch('http://192.168.0.108/capstone/tracsystem/backend/send_otp.php', {
        method: 'POST',
  ...
  const handleOtpSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch('http://192.168.0.108/capstone/tracsystem/backend/verify_otp.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
