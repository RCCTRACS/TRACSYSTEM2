...
export function BulkUploadStudent({
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      Papa.parse<Student>(text, {
        ...
        skipEmptyLines: true,
        complete: (results) => {
          // Expected headers based on your template
          ...
          const parsed: Student[] = results.data.map((row: any) => ({
  ...
  return (
    <DialogContent className="sm:max-w-[700px] bg-popover p-6 rounded-xl shadow-md">
      ...
      <div className="space-y-6 mt-4">
        ...
        {/* Preview */}
        {preview.length > 0 && (
          <div className="max-h-60 overflow-y-auto border p-3 rounded-lg bg-gray-50 text-sm shadow-inner">
            <p className="font-semibold text-black mb-2">Preview:</p>
            <div className="grid grid-cols-7 font-bold border-b pb-1 mb-1 text-[#3E1F0F]">
            ...
            {preview.map((s, idx) => (
              <div
                key={idx}
                className="grid grid-cols-7 gap-2 py-1 border-b last:border-0"
              >
